rundir <- if (length(commandArgs(TRUE)) >= 1) commandArgs(TRUE)[1] else "scenA_v6_replication"
...
for (m in c("sbart","rsf","ph"))
  cat(sprintf("%-15s %10.5f %10.5f %10.5f\n", nm[m],
              mean(amse[,m]), if (R > 1) sd(amse[,m]) else NA, median(amse[,m])))
cat("\n--- same, restricted to (0, t95] ---\n")
for (m in c("sbart","rsf","ph"))
  cat(sprintf("%-15s %10.5f %10.5f %10.5f\n", nm[m],
              mean(amse95[,m]), if (R > 1) sd(amse95[,m]) else NA, median(amse95[,m])))
...
if (!is.null(res[[1]]$own_width)) {
  ow <- t(sapply(res, `[[`, "own_width"))
  if (R == 1) ow <- matrix(ow, 1, dimnames = list(NULL, c("sbart","ph","rsf")))
  cat("\n=== mean width of each method's 95% pointwise interval ===\n")
  cat(sprintf("%-34s %14s %8s %12s\n", "", nm["sbart"], nm["rsf"], nm["ph"]))
  cat(sprintf("%-34s %14.4f %8.4f %12.4f\n", "mean interval width",
              mean(ow[,"sbart"]), mean(ow[,"rsf"]), mean(ow[,"ph"])))
